...
function AuthGate({ onAuth }){
  ...
  const handleLogin = async (e)=>{
    ...
    localStorage.setItem(LS_USER_KEY, email.trim());
    onAuth(email.trim());
  };
...
function BadgeCard({ unlocked, title, imgSrc }){
  return (
    <div className={["relative rounded-2xl p-4 border backdrop-blur",
      unlocked ? "border-cyan-400/40 bg-slate-900/60" : "border-slate-700 bg-slate-900/40"].join(" ")}
    >
  ...
  return (
    <div className="bg-slate-900/60 border border-slate-700 rounded-2xl p-5">
        ...
        {QUIZ.questions.map((q, qi)=> (
          <div key={qi} className="">
            ...
            <div className="mt-2 grid gap-2">
              {q.a.map((opt, idx)=> (
                <label key={idx} className={["cursor-pointer rounded-xl border p-3 text-sm",
                  answers[qi]===idx ? "border-cyan-400 bg-cyan-400/10" : "border-slate-700 hover:border-slate-600"].join(" ")}
                >
      ...
      <div className="mt-6 flex items-center gap-3">
        ...
        {submitted && (
          <div className={["text-sm font-bold", score >= QUIZ.passScore ? "text-emerald-400" : "text-rose-400"].join(" ")}>Sua nota: {score}%</div>
        )}
      </div>
    </div>
  );
